...
# Plotting routine 
#####################################
def gui_elecfraction_plots(irun, iz, iZZ, idens, PlotType, PlotDict, idisplay):
    ...
    if PlotType < 2:  # 1D plots
        ...
        stringlabel = 'Total'
        ax.plot(xx, Q1D[:,-1], label = stringlabel, color = 'black', linestyle = 'solid')

        for iion in range (len(Q1D[0,:])-1):
            if iion < len(ElementNames) - 1:
                stringlabel = '%s'%(ElementNames[iion].decode('utf-8'))
            elif iion == len(ElementNames) - 1:
                stringlabel = 'Total Prim'
            elif iion == len(ElementNames) :
                stringlabel = 'Total Metal'
            elif iion == len(ElementNames) + 1:
                stringlabel = 'Total'
            ax.plot(xx, Q1D[:,iion], label = stringlabel)       
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, bbox_to_anchor=(1.02, 0., 1.2, 1), loc=3, borderaxespad=0., ncol = 1)
